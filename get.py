...
def getInfluxDBClient():
    client = InfluxDBClient(host=INFLUXDB_HOST, port=INFLUXDB_PORT, username=INFLUXDB_USER, password=INFLUXDB_PASS)
...
def iterateStations(access_token):
    ...
    for station in stations:
        ...
        for substation in station['modules']:
            ...
            if 'Wind' in substation['data_type']:
...
def fetchMeasurements(access_token, device_id, module_id, measurement_type, station_name, module_name, client, last_update):
    ...
    result = client.query(get_latest_timestamp_query, database=INFLUXDB_DB)
